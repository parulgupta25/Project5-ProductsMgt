...
const loginUser = async (req, res) => {
    try {
        let data = req.body
        if (!isValidBody(data)) return res.status(400).send({ status: false, message: "Invalid Parameters, Enter login credentials" })

        let { email, password } = data
        if (!email) return res.status(400).send({ status: false, message: "Enter the email" })
        if (!password) return res.status(400).send({ status: false, message: "Enter the password" })

        if (!isValidEmail(email)) return res.status(400).send({ status: false, message: "email is invalid" })
        if (!isValidPassword(password)) return res.status(400).send({ status: false, message: "password is invalid" })

        let user = await userModel.findOne({ email: email })
        if (!user) return res.status(400).send({ status: false, message: "provide the correct email" })
